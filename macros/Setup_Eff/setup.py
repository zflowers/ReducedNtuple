...
def write_sh_sms(Cut,Dir,File,Tag):
    ...
    fsrc.write('executable = execute_script_eff.sh \n')
    ...
    fsrc.write('Arguments = \"-cut=$(CUT) -tag=$(TAG) -dir=$(DIR) -filename=$(FILENAME) -num= --eff\" \n')
    fsrc.write('transfer_input_files = '+path+'config_eff.tgz \n')
...
os.system("mkdir -p "+path+"config_eff")
os.system("cp "+path+"Eff_Nano_Hist.x "+path+"config_eff/")
os.system("cp /stash/user/zflowers/cmssw-sandbox/cmssw_setup.sh "+path+"config_eff/")
os.system("tar -C "+path+"config_eff/../ -czvf config_eff.tgz config_eff/")
