...
store = "/stash/user/zflowers/CMSSW_10_2_20_UL/src/ReducedNtuple/macros/EFF/"
sleep_time = 60
...
with open("/stash/user/zflowers/CMSSW_10_2_20_UL/src/ReducedNtuple/macros/Setup_Eff/Eff.txt") as cut_handle:
    for cut_line in cut_handle:
        if(cut_line.startswith('#')):
            continue;
        Cut.append(cut_line.replace('\n',''))
        with open("/stash/user/zflowers/CMSSW_10_2_20_UL/src/ReducedNtuple/macros/Setup_Eff/Dir.txt") as dir_handle:
            for dir_line in dir_handle:
                Dir.append(dir_line.replace('\n',''))
                for filename in os.listdir("/stash/user/zflowers/CMSSW_10_2_20_UL/src/ReducedNtuple/macros/Setup_Eff/"+dir_line.replace('\n','')+"/"):
                    File.append(filename.replace('.txt',''))
                    with open(os.path.join("/stash/user/zflowers/CMSSW_10_2_20_UL/src/ReducedNtuple/macros/Setup_Eff/"+dir_line.replace('\n','')+"/",filename),'r') as tag_handle:
